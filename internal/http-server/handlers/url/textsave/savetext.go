...
// 			// "Точка" записи данных в db
// 			// urlSaver это объект типа URLStorage (переданный при создании хендлера из main)
// 			// реализующий интерфейс URLtextSaver , т.к. имеет метод SaveURL
...
//
// Хендлер принимает параметром интерфейс URLtextSaver
...
func New(log *slog.Logger, urlSaver URLtextSaver) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			...
			// "Точка" записи данных в db
			// urlSaver это объект типа URLStorage (переданный при создании хендлера из map.go)
			// реализующий интерфейс URLtextSaver , т.к. имеет метод SaveURL
			err = urlSaver.SaveURL(url, alias)
